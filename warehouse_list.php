...
<? 
				$check_if_empty=mysql_num_rows(mysql_query("SELECT * FROM warehouse"));
				
				if($check_if_empty!=0){
				$mysql_s=mysql_query("SELECT * FROM warehouse");
				while($red=mysql_fetch_array($mysql_s)){ $br++; 
				$prdd=$red['prod_id'];
				$thiss=mysql_fetch_array(mysql_query("SELECT * FROM products WHERE id = '$prdd'"));
					
				$category_all = "";
			if($br%2==0) {$bgcolor = "#F4F4F4";} else {$bgcolor = "#FFFFFF";}
			if($thiss[cat_id] != 0) {
			$con2 = mysql_query("SELECT name FROM categories WHERE id = $thiss[cat_id]"); 
			$cat_name = mysql_fetch_assoc($con2); 
			$category_all = $cat_name['name']; }
			if($thiss[subcat_id] != 0) {
			$con2 = mysql_query("SELECT name FROM sub_categories WHERE id = $thiss[subcat_id]"); $cat_name = mysql_fetch_assoc($con2); 
			$category_all .= " > ".$cat_name['name']; }
			if($thiss[subsubcat_id] != 0) {
			$con2 = mysql_query("SELECT name FROM sub_sub_categories WHERE id = $thiss[subsubcat_id]"); $cat_name = mysql_fetch_assoc($con2); 
			$category_all .= " > ".$cat_name['name']; }
			...
			<td><b><?php echo "$thiss[name]"; ?></b><br><span style="font-size: 12px"><?php echo $category_all;  ?> </span> </td>
			<td><? if($thiss['promo']==1){ echo "<img src='other/promo.gif' width='35px' />"."$thiss[promo_price]"."лв. (Стара цена:".$thiss['price']."лв.)";}else{ ?><?php echo "$thiss[price]"; ?> лв.<? } ?></td>
			<td><center><? echo $red[quantity]; if($thiss[measure]=="на литър") echo " литра";
			if($thiss[measure]=="на брой") echo " броя";if($thiss[measure]=="на метър") echo " метра";if($thiss[measure]=="на килограм") echo " килограма"; ?></center></td>
			<td><center><a class="gray" style=" color: #464661;
    padding: 7px 15px;" href="ware_prod_edit.php?id=<?php echo "$red[id]"; ?>">Управление</a></center></td>
		</tr>
